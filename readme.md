...
//The target market being Sogyo employees the webshop//(use commas) can get away with assuming its userbase uses the latest version of Chrome, Firefox, Edge and other up to date browsers. This means that even though it's commonly advised to support as many older browsers as possible, Sogyo Adventure has the freedom to use the latest web technology. This includes (but is not limited to):
* Flexbox
* CSS grid
* CSS variables
* JavaScript modules
...
1. Define a function on the top level of //your script// (what script?) that will act as the event listener for the "order" buttons. Name this function `orderButtonClicked`. Register it on every button using `document.querySelectorAll()` with a CSS selector as the first argument, iterating over all matching elements and finally calling `.addEventListener()` of type `"click"`. //But where is `orderButtonClicked` being called?
2. Add a call to `console.log()`to the body of `orderButtonClicked`. Open the developer tools in your browser. //Click on the order the buttons.// (the the?) Is the call properly logged? If not: debug!
...
5. If there are no more family tickets available for a certain attraction, the order button should be disabled.
